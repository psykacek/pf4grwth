...
All data which were used in the publication "Critical Event Prediction in Shake Flask Cultivations With Non-linear Bayesian Estimation", (Pretzner <i>et al.</i> 2021) are available in the folder ./growthprofiles/. The folder contains
a description with experimental protocol and duration of measurements in the accompanying signal files (<i>experiment name</i>\_bm.csv for the biomass data and <i>experiment name</i>\_o2.csv for the oxygen concentrations). We provide here the calibrated biomass values and oxygen concentration readouts scaled by 10.   
